...
const RecommendedMovies = ({movieTitles}) => {
  ...
  const fetchMovie = async (title) => {
    const encodedTitle = encodeURIComponent(title);const url = `https://api.themoviedb.org/3/search/movie?query=${encodedTitle}&include_adult=false&language=en-US&page=1`;
    ...
    useEffect(() => {
        const loadMovies = async () => {
            ...
            setMovie(results.filter(Boolean));
            setLoading(false);
        };
  ...
  return (
    <div className='grid grid-cols-2 gap-4 md:grid-cols-5'>{movies.map((movie) => (
